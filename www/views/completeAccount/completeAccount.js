    ...
    if (angular.isDefined(user)) {
      ...
      account.$loaded().then(function() {
        ...
        if (account.length > 0) {
          ...
          accounts.$add({
            email: user.email,
            uname: user.uname, //uname is the name that will displayed when the student logs in
            ...
            Utils.message(Popup.successIcon, Popup.accountCreateSuccess)
              .then(function() {
                ...
                $localStorage.uname = user.uname; //add student display name in firebase database
